 ...
 **(4)** In the way I've constructed this table, you will notice that the sum of the rows are the total frequecy of the partitions a.j: ![alt text][table_pic2]
...
Here you'll notice that I put a special case for when the probability is 0 because log2(0) is infinity.  So the ifelse catches it here and replaces it with 0.  One case where this will happen is if d and a are the same vector.
...
**_Notice now that table x has the frequencies and table y has the component pieces of E(a.j)!!_**  Therefore, ```rowSums(y)```, will now give you E(a.i)._  This fact will be useful in (7).

**(6)** We need one last thing which is the weights for E(a.j).  Since table x still holds the frequencies, the weights are simply ```rowSums(x) / sum(x)```



**(7)*** What does ```sum(z * -1 * rowSums(y))``` do?  Well, since ```rowSums(y)``` is the entropy for a.j or E(a.j) and z has the weights, multiplying the two, taking the negative of it and summing it up will give you the total entropy of d given a or E(d,a).  

I did this in two steps.  First,  gives you the weights of each partition. If you remember from just above that ```rowSums(y)``` is each of the entropies then you'll see that ```sum(z * -1 * rowSums(y))``` multiplies the entropies by the weights and added them. (Oh and negative is done as well).
